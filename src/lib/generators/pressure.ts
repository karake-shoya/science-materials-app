  ...
  generate(count: number): QuestionData[] {
    ...
    for (let i = 1; i <= count; i++) {
      ...
      if (qType === 1) {
        ...
        qText = `問${i}. 面積が ${areaCm2}cm² の面に、垂直に ${forceN}N の力がはたらいています。このとき、この面にかかる圧力は何Paですか。`;
        ...
        qText = `問${i}. 面積 ${areaCm2}cm² の面に ${pressure}Pa の圧力が加わっています。この面を垂直におしている力の大きさは何Nですか。`;
        ...
        qText = `問${i}. ${forceN}N の力を加えたとき、面に ${pressure}Pa の圧力が生じました。この力がおしている面積は何cm²ですか。`;
