  ...
  generate(count: number, format: GeneratorFormat): QuestionData[] {
    ...
    for (let i = 1; i <= count; i++) {
      ...
      if (qType === 1) {
        ...
        qText = `問${i}. ${voltage}V の電圧をかけたとき、${current}A の電流が流れる電熱線があります。この電熱線の電力は何Wですか。`;
        ...
        qText = `問${i}. 消費電力が ${power}W の電熱線に、${minutes}分間電流を流しました。このときに発生した熱量は何Jですか。`;
        ans = joule.toString();
        unit = "J";
      } else {
        // Wh = W * h
        ...
        qText = `問${i}. 消費電力が ${power}W の電気器具を ${hours}時間使用しました。このときの電力量は何Whですか。`;
        ans = wh.toString();
        unit = "Wh";
      }
