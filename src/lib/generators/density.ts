  ...
  generate(count: number): QuestionData[] {
    ...
    for (let i = 1; i <= count; i++) {
      ...
      if (qType === 1) {
        ...
        qText = `問${i}. 質量が ${mass}g、体積が ${volume}cm³ の物質があります。この物質の密度は何g/cm³ですか。小数第1位まで答えなさい。`;
        ...
        const mass = Math.round(material.density * volume * 10) / 10;
        qText = `問${i}. 密度が ${material.density}g/cm³ の${material.name}があります。この${material.name}の体積が ${volume}cm³ のとき、質量は何gですか。`;
        ans = mass.toFixed(1);
        unit = "g";
      } else {
        // 体積 = 質量 / 密度
        ...
        const mass = Math.round(material.density * volume * 10) / 10;
        qText = `問${i}. 密度が ${material.density}g/cm³ の${material.name}が ${mass}g あります。この${material.name}の体積は何cm³ですか。`;
        ans = volume.toString();
        unit = "cm³";
      }
